...
## Usage
```sh
beeper send file [<localPath>] [flags]
```
## Arguments

| Name | Description |
| --- | --- |
| `[<localPath>]` | Local file path to upload. Used when --file is omitted. |
...
| `--caption=STRING` | Optional caption for file messages |
| `--filename=STRING` | Override displayed filename |
| `--mime=STRING` | Override MIME type |
| `--ptt` | Send audio as a voice note Default: `false`. |
