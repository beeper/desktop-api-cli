...
## Usage
```sh
beeper targets list (targets ls,target list,target ls) [flags]
```
## Aliases
...
Default JSON output is an object containing the `targets` field.
Use `--json --results-only` to emit only `targets`.
