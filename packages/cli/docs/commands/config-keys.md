...
## Usage
```sh
beeper config keys (config list-keys,config names) [flags]
```
## Aliases

- `beeper config list-keys`
- `beeper config names`

## JSON Output

Default JSON output is an object containing the `keys` field.
Use `--json --results-only` to emit only `keys`.
