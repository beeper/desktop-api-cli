...
| `--chat <value>` | Limit to chat selector Repeatable. |
| `--force` | Re-export completed chats Default: `false`. |
| `--limit-chats <value>` | Maximum chats to export |
| `--limit-messages <value>` | Maximum messages per chat |
| `--max-participants <value>` | Maximum participants in chat.json Default: `500`. |
| `--no-attachments` | Skip downloading attachments Default: `false`. |
| `--out <value>` | Export directory Default: `beeper-export`. |
