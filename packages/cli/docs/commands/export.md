# beeper export
Export accounts, chats, messages, transcripts, and attachments
## Usage
```sh
beeper export [flags]
```
...
| `--chat=STRING` | Limit to chat selector Repeatable. |
| `--force` | Re-export completed chats Default: `false`. |
| `--limit-chats=INTEGER` | Maximum chats to export |
| `--limit-messages=INTEGER` | Maximum messages per chat |
| `--max-participants=500` | Maximum participants in chat.json Default: `500`. |
| `--no-attachments` | Skip downloading attachments Default: `false`. |
| `--out="beeper-export"` | Export directory Default: `beeper-export`. |
