# beeper auth services
List supported account login services and bridges
## Usage
```sh
beeper auth services (auth bridges) [flags]
```
## Aliases
...
Default JSON output is an object containing the `services` field.
Use `--json --results-only` to emit only `services`.

## Flags

| Name | Description |
| --- | --- |
| `--markdown` | Output a Markdown table Default: `false`. |
