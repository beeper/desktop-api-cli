...
| Name | Description |
| --- | --- |
| `--allow-tool <value>, --tool` | Tool or command allowlist Repeatable. |
| `--allow-write` | Allow write-risk MCP tools Default: `false`. |
| `--list-tools` | Print enabled MCP tools as JSON and exit Default: `false`. |
| `--max-output-bytes <value>` | Maximum stdout/stderr bytes captured per tool call Default: `102400`. |
| `--timeout-seconds <value>` | Per-tool subprocess timeout Default: `60`. |
