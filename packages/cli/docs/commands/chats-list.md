...
## Usage
```sh
beeper chats list (chats ls,chat list,chat ls,ls,list) [flags]
```
## Aliases
...
| `--limit=50` | Maximum chats to print Default: `50`. |
...
| `--query=STRING` | Optional chat lookup query |
| `--type=STRING, --chat-type` | Only direct messages, group chats, or all chats Values: `single`, `group`, `any`. |
