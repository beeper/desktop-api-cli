...
Finish email sign-in for a target
## Usage
```sh
beeper auth email response [flags]
```
## Flags

| Name | Description |
| --- | --- |
| `--code=STRING` | Email verification code Required. |
| `--setup-request-id=STRING` | Setup request ID from auth email start Required. |
