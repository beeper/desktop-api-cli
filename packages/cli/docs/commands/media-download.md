...
## Usage
```sh
beeper media download (media dl,download,dl) [<url>] [flags]
```
## Aliases
...
| Name | Description |
| --- | --- |
| `[<url>]` | Media URL to download. Use --id with --chat to download from a message. |

## Flags

| Name | Description |
| --- | --- |
| `--out=".", --output` | Output directory or file; - streams to stdout Default: `.`. |
| `--chat=STRING` | Chat selector for --id message lookup |
| `--id=STRING` | Message ID containing media |
| `--index=1` | Attachment index to download, 1-based Default: `1`. |
| `--poster` | Download attachment poster image when available Default: `false`. |
