...
## Usage
```sh
beeper targets logs (target logs) [<name>] [flags]
```
## Aliases
...
| Name | Description |
| --- | --- |
| `--lines=200` | Lines to print from each log file Default: `200`. |
| `--files=5` | Desktop log files to print, newest first Default: `5`. |
| `--all` | Print all matching log files instead of only recent files Default: `false`. |
