...
| Name | Description |
| --- | --- |
| `--chat=STRING` | Chat ID to subscribe to; defaults to all chats Repeatable. |
| `--exclude-type=STRING` | Drop events of these types Values: `chat.upserted`, `chat.deleted`, `message.upserted`, `message.deleted`, `message.stream`. Repeatable. |
| `--include-type=STRING` | Only forward events of these types Values: `chat.upserted`, `chat.deleted`, `message.upserted`, `message.deleted`, `message.stream`. Repeatable. |
| `--webhook=STRING` | Forward each event to this URL as POST |
| `--webhook-allow-private` | Accepted for compatibility; Beeper watch allows private webhook URLs Default: `false`. |
| `--webhook-queue=64` | Maximum pending webhook deliveries Default: `64`. |
| `--webhook-secret=STRING` | HMAC-SHA256 secret for X-Beeper-Signature and X-Wacli-Signature |
