# beeper targets runtime restart
Restart a local server runtime
## Usage
```sh
beeper targets runtime restart (target runtime restart) [<name>] [flags]
```
## Aliases

- `beeper target runtime restart`
